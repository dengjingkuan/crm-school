...
class ClassList(models.Model):
    ...
    班级表
        例如：
        python开发 面授 1期 12000 2020-1-1 2020-12-1
    """
    ...
    class_teacher = models.ForeignKey(verbose_name="班主任", to='UserInfo', related_name='classes',
                                      on_delete=models.CASCADE, limit_choices_to={"depart__title": "教育质量部"})
    teach_teacher = models.ManyToManyField(verbose_name="讲师", to='UserInfo', related_name='teach_classes', blank=True,
                                           limit_choices_to={"depart__title__in": ['Python教学部', 'Linux教学部']})

    memo = models.TextField(verbose_name="说明信息", null=True, blank=True)
...
class PaymentRecord(models.Model):
    ...
    confirm_user = models.ForeignKey(verbose_name="审批人", to='UserInfo',
                                     related_name='confirms', null=True, blank=True, on_delete=models.CASCADE)
...
class ScoreRecord(models.Model):
    ...
    积分记录
    """
    student = models.ForeignKey(to='Student', verbose_name='学生姓名', on_delete=models.CASCADE)
    content = models.TextField(verbose_name='理由')
    score = models.IntegerField(verbose_name='分值', help_text='违纪扣分, 表扬加分')
